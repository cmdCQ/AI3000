...
INGEST_EMBED_BATCH_SIZE = int(os.environ.get("RAG_INGEST_EMBED_BATCH_SIZE", "16"))
INGEST_STORE_BATCH_SIZE = int(os.environ.get("RAG_INGEST_STORE_BATCH_SIZE", "64"))
...
def init_services():
    ...
    emb_api_key = os.environ.get("DEEPSEEK_API_KEY") or emb_cfg.get("remote_api_key", "")
...
async def ingest(req: IngestRequest):
    ...
    try:
        ...
        # 2. 分批向量化 + 分批存储，避免大文本一次性撑爆内存
        pending_texts = []
        pending_embeddings = []
        pending_metadatas = []

        for i in range(0, len(chunks), INGEST_EMBED_BATCH_SIZE):
            chunk_batch = chunks[i : i + INGEST_EMBED_BATCH_SIZE]
            text_batch = [c["text"] for c in chunk_batch]
            metadata_batch = [
                {
            ...
            embedding_batch = embedder.embed_documents(text_batch, batch_size=INGEST_EMBED_BATCH_SIZE)

            pending_texts.extend(text_batch)
            pending_embeddings.extend(embedding_batch)
            pending_metadatas.extend(metadata_batch)

            if len(pending_texts) >= INGEST_STORE_BATCH_SIZE or (i + INGEST_EMBED_BATCH_SIZE) >= len(chunks):
                vector_store.add_chunks(req.category, pending_texts, pending_embeddings, pending_metadatas)
                pending_texts.clear()
