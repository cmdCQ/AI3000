...
const MHYS_METHOD_NAMES = {
  // `character` 写「字占」：与记录页/后台/前端下拉的叫法统一（2026-09-25 起字占按
  // 《梅花易数》原文分层取数：≤3 字笔画、4–10 字平仄、≥11 字只按字数）。
  time: '时间起卦', manual: '手动指定', character: '字占',
 ...
 *
 * 为什么非要有这一条（不是为了好看）：
 * * **字占**根本在前端做不了 —— 笔画表 6944 字在 `paipan/strokes.js`，四字以上要用的
 *   **平仄取数表**（26660 字）在 `paipan/pingze.js`，两张表前端都没有。
