...
function calcGua(upperNum, lowerNum, moveNum) {
  ...
  var benGua={
    ...
    name:hexName,
    lines:buildLines(upperNum,lowerNum,moveNum)
  };
...
function timeDivination() {
  ...
  // 年支数（干支编号：子1 丑2 ... 亥12）
  var yearZhiNum = ((y - 4) % 12 + 12) % 12 + 1;
  // 子时（23:00-00:59）日柱算次日
  var calcDay = sc.num === 1 ? lunarDay + 1 : lunarDay;
  var upperRaw = yearZhiNum + lunarMonth + calcDay;
  var lowerRaw = yearZhiNum + lunarMonth + calcDay + sc.num;
  var moveRaw = yearZhiNum + lunarMonth + calcDay + sc.num;
...
function manualDivination() {
  var u=parseInt(document.getElementById('manualUpper').value);
  var l=parseInt(document.getElementById('manualLower').value);
  var m=parseInt(document.getElementById('manualMove').value);
...
function renderMethodParams() {
  ...
  switch(currentMethod) {
    case 'time':
      h = '<div class="algo-text">起卦方法（月日数为农历）：\n上卦：(年支数 + 月数 + 日数) ÷ 8 所得余数为上卦\n下卦：(年支数 + 月数 + 日数 + 时支数) ÷ 8 所得余数为下卦\n动爻：(年支数 + 月数 + 日数 + 时支数) ÷ 6 所得余数</div>';
      break;
    case 'manual':
      h = '<div style="margin-top:0.6rem">'+
          '<div class="form-row"><label>上卦</label><select id="manualUpper"></select></div>'+
          '<div class="form-row"><label>下卦</label><select id="manualLower"></select></div>'+
          '<div class="form-row"><label>动爻</label><select id="manualMove"></select></div>'+
          '</div>';
      // 延迟填充下拉
      setTimeout(fillManualSelects, 50);
      break;
    case 'num1':
      h += '<div style="margin-top:0.6rem">'+
          '<div class="num-input-row"><input type="text" id="num1Input" placeholder="连续输入数字，如 386 或 1234" inputmode="numeric" style="flex:1"><label class="cb-inline"><input type="checkbox" id="num1AddShichen" checked> 动爻加时辰</label></div>'+
          '<div class="algo-text">起卦算法：取输入的每一位数字，平分为前后两半，前一半之和 ÷8 得余数为上卦，后一半之和 ÷8 得余数为下卦，上下卦数相加 ÷6 得余数为动爻。若数字个数为奇数，前部分比后部分少一个。</div>'+
          '</div>';
      break;
    case 'num2':
      h += '<div style="margin-top:0.6rem">'+
          '<div class="num-input-row"><input type="text" id="num2Input" placeholder="输入三个数字，如 386" maxlength="3" inputmode="numeric" style="flex:1;text-align:center"><label class="cb-inline"><input type="checkbox" id="num2AddShichen" checked> 动爻加时辰</label></div>'+
          '<div class="algo-text">起卦算法：第1个数 ÷8 得余数为上卦，第2个数 ÷8 得余数为下卦，第3个数 ÷6 得余数为动爻。</div>'+
          '</div>';
      break;
    case 'auto':
      h = ''; // 不显示输入框和说明
...
function fillManualSelects() {
  var selU=document.getElementById('manualUpper');
  var selL=document.getElementById('manualLower');
  var selM=document.getElementById('manualMove');
  if(!selU||selU.options.length>0) return;
  for(var i=1;i<=8;i++){
    var t=TRIGRAMS[i];
    selU.innerHTML+='<option value="'+i+'">'+t.symbol+' '+t.full+'</option>';
    selL.innerHTML+='<option value="'+i+'">'+t.symbol+' '+t.full+'</option>';
  }
  for(var j=1;j<=6;j++) selM.innerHTML+='<option value="'+j+'">第 '+j+' 爻</option>';
}
...
async function startDivination(e) {
  ...
  // ===== 1. 验证（统一在算法前） =====
  if (currentMethod === 'num1') {
    var raw = document.getElementById('num1Input').value.trim();
    if (!raw || raw.replace(/[^0-9]/g,'') === '') { alert('请输入数字'); resetBtn(); return; }
  }
  if (currentMethod === 'num2') {
    var raw2 = document.getElementById('num2Input').value.trim();
    var digits = raw2.replace(/[^0-9]/g,'').split('');
    if (digits.length !== 3) { alert('请输入三个数字（0-9）'); resetBtn(); return; }
  }

  // ===== 2. 计算卦象（各方法不同） =====
  var result;
  switch(currentMethod) {
    case 'time': result = timeDivination(); break;
    case 'manual': result = manualDivination(); break;
    case 'num1': result = num1Divination(); break;
    case 'num2': result = num2Divination(); break;
    case 'auto': result = autoDivination(); break;
    default: resetBtn(); return;
  }

  // ===== 3. 统一：topic / 起卦时间 / 农历 / 跳转 =====
  ...
  // 事项为空 → 临时排盘，不保存记录
  if (!topic) {
    localStorage.setItem('mhys_result', JSON.stringify(result));
    window.location.href = '/mhys/result.html?v=3';
    return;
  }

  try {
    var resp = await fetch('/api/mhys-records', {
      method: 'POST',
    ...
    if (resp.ok) {
      window.location.href = '/mhys/result.html?v=3&id=' + saved.id;
    } else {
      window.location.href = '/mhys/result.html?v=3';
    }
  } catch(e) {
